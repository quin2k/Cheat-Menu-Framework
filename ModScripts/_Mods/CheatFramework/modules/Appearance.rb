...
module MenuFramework
  module SUBMENU
    ...
    register_command(
      ...
      type:   :action,
      key:    "Pubes",
      label:  "modules/character:commands/appearance/pubes",
      hide:   -> { $game_player.actor.stat["PubicHairVag"] == 0 || $game_player.actor.stat["PubicHairAnal"] == 0 },
      action: -> { 
        FrameworkUtils.custom_state_edit("PubicHairVag",-4) if actor.stat["PubicHairVag"] != 0
        FrameworkUtils.custom_state_edit("PubicHairAnal",-4) if actor.stat["PubicHairAnal"] != 0
        $game_player.actor.refresh_chs
      },
      order:  20
    )
    register_command(
